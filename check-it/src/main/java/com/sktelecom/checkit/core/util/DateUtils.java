...
//				}
	...
	/**
     * 시작월~종료월 까지 월 리스트 반환  from에서 to까지 날짜의 월 리스트 반환한다.(날짜형태:yyyyMM)
     * @param String from
     * @param String to
     * @return List
     */
    public static List getMonthDiff(String from, String to){
		...
		cal.set ( Integer.parseInt(to.substring(0, 4)), Integer.parseInt(to.substring(4, 6))-1, 1 ); //종료 날짜 셋팅
		String endDate = dateFormat.format(cal.getTime());
		
		cal.set ( Integer.parseInt(from.substring(0, 4)), Integer.parseInt(from.substring(4, 6))-1, 1 ); //시작 날짜 셋팅
		String startDate = dateFormat.format(cal.getTime());    	
		
		if(startDate.equals(endDate)) {
			monthList.add(dateFormat.format(cal.getTime()));
		} else {
			int i = 0;
